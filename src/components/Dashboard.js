...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      try {
        const response = await axios.get("https://13.60.196.243:5000/api/files/documents");
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post("https://13.60.196.243:5000/api/files/upload", formData, {
  ...
  const handlePdfSign = async (fileId, index) => {
    ...
    try {
      ...
      const response = await axios.post("https://13.60.196.243:5000/api/files/sign-pdf", {
        fileId,
                    ...
                    <TableRow key={index}>
                      ...
                      <TableCell>
                        <Button
                          variant="contained"
                          color="primary"
                          href={`https://13.60.196.243:5000/uploads${item.signedFilePath.replace("/uploads", "")}`}
                          target="_blank"
